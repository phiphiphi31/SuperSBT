...
class DIFFUSIONTRACK(nn.Module):
    ...
    def __init__(self, cfg, encoder, hidden_dim,
        ...
        if cfg.MODEL.HEAD.NUM_BOTTLENECK == 1:
            self.bottleneck = nn.Linear(encoder.num_channels, hidden_dim)  # the bottleneck layer
        else:
            num_bottleneck = cfg.MODEL.HEAD.NUM_BOTTLENECK
            self.bottleneck1 = nn.Linear(encoder.num_channels, hidden_dim) # the bottleneck layer
            #self.bottleneck_series = _get_clones(bottleneck, num_bottleneck)
            self.bottleneck2 = nn.Linear(encoder.num_channels, hidden_dim)
            self.bottleneck3 = nn.Linear(encoder.num_channels, hidden_dim)
        self.num_diffmodel = cfg.MODEL.HEAD.NUM_DIFFMODEL
        if cfg.MODEL.HEAD.NUM_DIFFMODEL == 1:
            self.head = DynamicHead(cfg, [stride, hidden_dim])
        else:
            num_diffmodel = cfg.MODEL.HEAD.NUM_DIFFMODEL
            self.head1 = DynamicHead(cfg, [stride, hidden_dim])
            #self.head_series = _get_clones(head, num_diffmodel)
            self.head2 = DynamicHead(cfg, [stride, hidden_dim])
            self.head3 = DynamicHead(cfg, [stride, hidden_dim])
        ...
        # build diffusion
        timesteps = 1000
        sampling_timesteps = 2  # cfg.MODEL.DiffusionDet.SAMPLE_STEP
    ...
    def forward_decoder_multi(self, xz, boxes, t):
        output={}
        all = xz[-1]
        xz_mem1 = all['all'][3]
        xz_mem2 = all['all'][7]
        ...
        B, _, _ = xz_mem1.shape
        dec_mem1 = xz_mem1[:, 0:self.num_patch_x * self.num_frames]
        dec_mem2 = xz_mem2[:, 0:self.num_patch_x * self.num_frames]
        dec_mem3 = xz_mem3[:, 0:self.num_patch_x * self.num_frames]

        # align the dimensions of the encoder and decoder
        if dec_mem1.shape[-1] != self.hidden_dim:
            dec_mem1 = self.bottleneck1(dec_mem1)  # [B, NL, C]
            dec_mem2 = self.bottleneck2(dec_mem2)  # [B, NL, C]
            dec_mem3 = self.bottleneck3(dec_mem3)  # [B, NL, C]

       # dec_mem = dec_mem.permute(1,0,2)  #[NL,B, C]
        if self.num_diffmodel ==1:
            outputs_class, outputs_coord = self.head([dec_mem1, dec_mem2, dec_mem3], boxes, t, None)
            ...
            outputs_class1, outputs_coord1 = self.head1(dec_mem1, boxes, t, None)
            bboxes_prev1 = outputs_coord1[-1].detach()
            outputs_class2, outputs_coord2 = self.head2(dec_mem2, bboxes_prev1, t, None)
            bboxes_prev2 = outputs_coord2[-1].detach()
            outputs_class3, outputs_coord3 = self.head3(dec_mem3, bboxes_prev2, t, None)

            output['pred_logits'] = outputs_class3[-1]
            output['pred_boxes'] = outputs_coord3[-1]

            if self.deep_supervision:
                output['aux_outputs'] = [{'pred_logits': outputs_class1[0, ], 'pred_boxes': outputs_coord1[0, ]},
                                     {'pred_logits': outputs_class1[1, ], 'pred_boxes': outputs_coord1[1, ]},
                                     {'pred_logits': outputs_class2[0,], 'pred_boxes': outputs_coord2[0,]},
                                     {'pred_logits': outputs_class2[1,], 'pred_boxes': outputs_coord2[1,]},
                                     {'pred_logits': outputs_class3[0,], 'pred_boxes': outputs_coord3[0,]},
                                     ]
    ...
    def forward_decoder_3dm(self, xz, boxes, t):
        output={}
        all = xz[-1]
        xz_mem1 = all['all'][self.encoder_feature[0]]
        xz_mem2 = all['all'][self.encoder_feature[1]]
        xz_mem3 = all['all'][self.encoder_feature[2]]
        ...
        outputs_class1, outputs_coord1 = self.head_series[0](dec_mem1, boxes, t, None)
        bboxes_prev1 = outputs_coord1[-1].detach()

        outputs_class2, outputs_coord2 = self.head_series[1](dec_mem2, bboxes_prev1, t, None)
        bboxes_prev2 = outputs_coord2[-1].detach()

        outputs_class3, outputs_coord3 = self.head_series[2](dec_mem3, bboxes_prev2, t, None)
    ...
    def forward_decoder(self, xz, boxes, t):
        ...
        outputs_class, outputs_coord = self.head(dec_mem, boxes, t, None)
        output = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}
    ...
    def inference_decoder_multi(self, xz, sequence=None, window=None, seq_format='xywh'):
        ...
        dec_mem2 = xz_mem2[:, 0:self.num_patch_x * self.num_frames]
        dec_mem3 = xz_mem3[:, 0:self.num_patch_x * self.num_frames]
        #  dec_mem4 = xz_mem4[:, 0:self.num_patch_x * self.num_frames]
        #  dec_mem5 = xz_mem5[:, 0:self.num_patch_x * self.num_frames]
        #  dec_mem6 = xz_mem6[:, 0:self.num_patch_x * self.num_frames]
        ...
        output = self.ddim_sample(backbone_x_feats = [dec_mem1, dec_mem2, dec_mem3])
        return output
    ...
    def model_predictions(self, backbone_x_feats, images_whwh, x, t, clip_x_start=True):
        ...
        outputs_class, outputs_coord = self.head(backbone_x_feats, x_boxes, t, None)
        ...
        return [pred_noise, x_start], outputs_class, outputs_coord
    ...
    def ddim_sample(self, backbone_x_feats,  clip_denoised=True):
        ...
        for time, time_next in time_pairs:
            ...
            preds, outputs_class, outputs_coord = self.model_predictions(backbone_x_feats, images_whwh, img, time_cond, clip_x_start=clip_denoised)
        ...
        if self.sampling_timesteps > 1:
            ...
            output = {'pred_logits': scores_per_image, 'pred_boxes': box_pred_per_image}
                      #'ensemble_logits':ensemble_logits[-1], 'ensemble_boxes': ensemble_boxes[-1]}
        else:
            pred_logits = torch.sigmoid(outputs_class[-1])
            pred_boxes = outputs_coord
            output = {'pred_logits': pred_logits[-1], 'pred_boxes': pred_boxes[-1]}
