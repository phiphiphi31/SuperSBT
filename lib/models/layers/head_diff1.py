...
class Diffusion_Predictor(nn.Module):
    ...
    def forward_inference(self, x):
        ...
        for time, time_next in time_pairs:
            ...
            t_embedding = self.time_mlp(time_cond)
            feat_diffused = torch.cat([x, noise_map], dim=1)
            t_embedding = t_embedding.unsqueeze(-1).unsqueeze(-1)
            feat_diffused_t = feat_diffused + t_embedding
            ...
            noise_map_raw = torch.zeros([b, 1, w, h], device=x.device)
            noise_map = noise_map_raw # 0.1 - 72.1, -0.5 - 72.6, zero-71.9
