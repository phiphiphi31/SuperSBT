...
class LTRTrainer(BaseTrainer):
    ...
    def cycle_dataset(self, loader):
        ...
        print_str = "Epoch Time: " + str(datetime.timedelta(seconds=epoch_time)) + '\n' \
                    + "total Data Time: %.5f" % (self.avg_date_time) + '\n' \
                    + "total GPU Trans Time: %.5f" % (self.avg_gpu_trans_time) + '\n'\
                    + "total Forward Time: %.5f" % (self.avg_forward_time)
