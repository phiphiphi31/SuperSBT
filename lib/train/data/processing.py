...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        # data for diffusion
        data["search_masks_diff"] = torch.zeros((1, self.output_sz["search"]//16, self.output_sz["search"]//16))
        x_index = (data["search_anno"][0][0] * (self.output_sz["search"]//16)).int().item()
        y_index = (data["search_anno"][0][1] * (self.output_sz["search"]//16)).int().item()
        w_index = int((data["search_anno"][0][2] * (self.output_sz["search"]//16)).round().item())
        h_index = int((data["search_anno"][0][3] * (self.output_sz["search"]//16)).round().item())
        data["search_masks_diff"][:, x_index:x_index+w_index, y_index:y_index+h_index ] = torch.randn((w_index, h_index))
        data["search_masks_diff"] = torch.where( data["search_masks_diff"] > 0.0, 1.0, 0.0 )
        data["search_masks_diff"] = [data["search_masks_diff"]]
